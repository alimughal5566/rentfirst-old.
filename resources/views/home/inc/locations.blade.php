                            ...
                            @if (isset($cities))
                                <div class="relative location-content">
                                    ...
                                    <div class="col-xl-12 tab-inner">
                                        <div class="row location-content-wrap">
                                            @foreach ($cities as $key => $items)
                                                <ul class="mx-auto cat-list {{ $ulCol }} {{ (count($cities) == $key+1) ? 'cat-list-border' : '' }}">
                                                    @foreach ($items as $k => $city)
                                                        <li class="text-center text-sm-left">
                                                            @if ($city->id == 999999999)
                                                                <a href="#browseAdminCities" id="dropdownMenu1"
                                    ...
                                    @if ($loc['showButton'])
                                        @if (!auth()->check() and config('settings.single.guests_can_post_ads') != '1')
                                            <a class="btn btn-lg btn-add-listing" href="#quickLogin"
                                               data-toggle="modal">
